...
db.serialize(() => {
    ...
    db.run(
        ...
        function (err) {
            ...
            db.run(
                `INSERT INTO diaries (title, content, cover_image, user_id, collection_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
                [diaryTitle, diaryContent, 'https://images.unsplash.com/photo-1490750967868-58cb75069ed6?auto=format&fit=crop&q=80&w=1000', userId, collectionId],
                function (err) {
            ...
            db.run(
                `INSERT INTO diaries (title, content, cover_image, user_id, collection_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, datetime('now', '-3 months'), datetime('now'))`,
                [diaryTitle2, diaryContent2, 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&q=80&w=1000', userId, collectionId],
                function (err) {
                    if (err) return console.error(err);
                    console.log(`✅ 已创建并在合集中的日记 ID: ${this.lastID}`);
                    db.close();
