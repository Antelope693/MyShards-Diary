...
export default function GlobalLinksPanel({ activeUsername }: GlobalLinksPanelProps) {
  ...
  const fetchLinks = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || '加载失败，请稍后再试');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleNavigate = (link: GlobalLink) => {
    // Legacy direct navigation fallback or strictly use dropdown now?
    // Requirement says "Click... dropdown appears". So handleNavigate is likely obsolete if we wire existing click to dropdown.
    // The previous implementation used handleNavigate for title click?
    // I will wire the main click to toggle Expanded User.
  ...
  return (
    <div className="fixed left-2 top-32 sm:left-4 lg:left-6 z-40 flex flex-col gap-4">
      ...
      >
        ...
        <span className="font-semibold text-gray-700 text-sm">我们</span>
      ...
      >
        <div className="relative rounded-2xl bg-white/95 backdrop-blur border border-gray-100 shadow-xl overflow-hidden max-h-[80vh] flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-indigo-500 font-semibold mb-1">MyShards · WE</p>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 text-sm custom-scrollbar">
            ...
            {loading ? (
              ...
              <div className="text-gray-500 text-sm py-4">暂时只有羚羊在书写，快来注册属于你的日记站。</div>
            ) : (
              <div className="space-y-3 pb-2">
                {filteredLinks.length === 0 ? (
                  <div className="text-sm text-gray-500 py-4">未找到匹配的伙伴，换个关键词试试？</div>
                ) : (
                      ...
                      <div key={`${link.username || link.url}`} className="w-full">
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mb-1 pr-6">
                            ...
                            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">{link.diary_count || 0} 篇</span>
                          </div>
                          ...
                          {link.latest_diary_at && (
                            <div className="flex items-center gap-2 text-xs text-gray-400">
                              ...
                              <span>{new Date(link.latest_diary_at).toLocaleDateString('zh-CN')} 更新</span>
                            </div>
                          )}
                        </button>
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-64 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                          <div className="bg-gray-50/50 rounded-xl border border-gray-100 p-2 grid grid-cols-2 gap-2">
                            <ActionButton icon={<LayoutDashboard size={14} />} label="查看空间" onClick={() => handleAction('space', link)} />
                            <ActionButton icon={<UserCheck size={14} />} label="我的关注" onClick={() => handleAction('following', link)} />
                            <ActionButton icon={<Star size={14} />} label="我的收藏" onClick={() => handleAction('collections', link)} />
                            <ActionButton icon={<Users size={14} />} label="关注我的" onClick={() => handleAction('followers', link)} />
                            <ActionButton icon={<Briefcase size={14} />} label="我的生涯" onClick={() => handleAction('career', link)} />
                            <ActionButton icon={<MoreHorizontal size={14} />} label="更多功能" onClick={() => handleAction('more', link)} />
                          </div>
                        </div>
                      </div>
                    );
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="font-semibold text-gray-700 text-sm">他们</span>
      ...
      >
        <div className="relative rounded-2xl bg-white/95 backdrop-blur border border-gray-100 shadow-xl overflow-hidden max-h-[80vh] flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-emerald-500 font-semibold mb-1">MyShards · THEY</p>
              <h3 className="font-title text-xl text-gray-800 flex items-center space-x-2">
          ...
          <div className="px-5 py-3 bg-gray-50/50 border-b border-gray-100 flex gap-2 shrink-0 overflow-x-auto no-scrollbar">
            <FilterChip active={activeFilter === 'random'} onClick={() => setActiveFilter('random')} icon={<Shuffle className="w-3 h-3" />} label="随机漫游" />
            <FilterChip active={activeFilter === 'newest'} onClick={() => setActiveFilter('newest')} icon={<Clock className="w-3 h-3" />} label="最新发布" />
            <FilterChip active={activeFilter === 'hot'} onClick={() => setActiveFilter('hot')} icon={<Flame className="w-3 h-3" />} label="热门收藏" />
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 text-sm custom-scrollbar min-h-[300px]">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-12 text-gray-400 space-y-3">
                ...
                <span className="text-xs">正在捕获信号...</span>
              </div>
            ) : (
