...
export default function ContentRenderer({ content }: ContentRendererProps) {
  // 先处理图片标记，将其转换为 HTML img 标签
  const processContent = (text: string): string => {
    // 将 [img:url:width%] 转换为 HTML img 标签
    return text.replace(/\[img:([^\]]+):(\d+)%\]/g, (_match, url, width) => {
      return `\n<img src="${url}" alt="日记图片" style="width: ${width}%; height: auto; display: block; margin: 1.5rem auto; border-radius: 0.5rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);" />\n`;
    });
  };
