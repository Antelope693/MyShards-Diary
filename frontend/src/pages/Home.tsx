...
import { diaryApi, greetingApi, Diary, usersApi, collectionsApi, collectsApi, Collection, GlobalLink } from '../api/client';
...
export default function Home() {
  ...
  // Decide active username: 
  // 1. URL param (highest priority) -> "Visiting someone's space"
  // 2. Logged User -> "My Space"
  // 3. Maintainer -> "Guest View"
  // 4. Loading state...
  ...
  useEffect(() => {
    // If username is in URL, we load immediately.
    if (username) {
    ...
    // If no username, verify auth first.
    if (authLoading) return;

    const target = user ? user.username : maintainerUsername;

    // Only reload if target changed or initial load
    if (activeUsername !== target) {
      setActiveUsername(target);
      loadData(target);
    } else if (items.length === 0 && !loading) {
      // Retry or initial? 
      // Usually activeUsername is '' initially, so it enters the block above.
      // But if we came from username url to root... 
      // activeUsername state holds 'username'. 
      // target is 'me'. They differ. So it reloads. Correct.
    }
  ...
  const loadData = async (targetUsername: string) => {
    ...
    try {
      ...
      if (e.response?.status === 404) setProfileError('未找到该作者');
    } finally {
      setLoading(false);
    }
  };
  ...
  const timelineGroups = useMemo(() => {
    ...
    return sortedYears.map(year => {
      // Find months in this year that have items
  ...
  return (
    <div className="relative min-h-screen pb-20">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4">
        ...
        <div className="text-center py-20 lg:py-24 space-y-6">
          ...
          <p className="font-body text-xl text-gray-500 font-light max-w-2xl mx-auto">
            {profileError || profile?.bio || '在仍有选择的时候，不要让自己后悔'}
          </p>
          <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/50 backdrop-blur border border-gray-200 text-sm text-gray-600 shadow-sm">
            ...
            <span>正在浏览 <b>{activeUsername}</b> 的时空</span>
          </div>
        </div>
        ...
        <div className="flex justify-center mb-16">
          <Card variant="glass" className="max-w-3xl text-center shadow-lg hover:shadow-xl transition-shadow">
            <p className="font-title text-xl text-gray-800 leading-relaxed">"{greeting}"</p>
          </Card>
        </div>

        {/* Unified Timeline Container or Other Views */}
        {view === 'timeline' ? (
            ...
            {pinnedDiaries.length > 0 && (
              <section className="mb-20">
                ...
                <div className="relative mb-10">
                  {/* Pinned Node - Aligned with Timeline */}
                  ...
                  <div className="flex items-center gap-3 px-2">
                    ...
                    <h2 className="font-title text-3xl font-bold text-gray-800">置顶回忆</h2>
                  </div>
                </div>
                              ...
                              {expandedCollectionIds.has((item.data as Collection).id) && (
                                <div className="mt-8 ml-4 md:ml-8 border-l-2 border-indigo-100 pl-8 space-y-8 animate-in slide-in-from-top-4 duration-300 fading-in">
                                  {collectionDiariesMap[(item.data as Collection).id]?.length ? (
                                    ...
                                    <div className="text-gray-400 text-sm italic py-4">加载中或无日记...</div>
                                  )}
                                </div>
                              )}
                            </div>
          ...
          <div className="max-w-3xl mx-auto min-h-[500px]">
            ...
            <div className="grid gap-4">
              ...
              {(view === 'following' ? followingList : followersList).length === 0 && (
                <div className="text-center py-20 text-gray-400">暂无相关用户</div>
              )}
            </div>
          </div>
        ) : view === 'collections' ? (
          <div className="max-w-3xl mx-auto min-h-[500px]">
            ...
            <div className="grid gap-6">
              ...
              {collectedDiaries.length === 0 && (
                <div className="text-center py-20 text-gray-400">暂无收藏的日记</div>
              )}
            </div>
          </div>
        ) : null}
      ...
      {/* Collection Modal */}
      {/* Removed selectedCollection state and CollectionModal component as per new inline expansion */}
    </div >
  );
}
