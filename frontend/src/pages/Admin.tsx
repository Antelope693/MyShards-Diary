...
export default function Admin({ managedUser, allowOwnerOverride }: AdminProps = {}) {
  ...
  const handleRecalculateStorage = async () => {
    ...
    try {
      ...
      alert(error?.response?.data?.error || '重新统计失败，请稍后再试');
    }
  };
      ...
      <div className="mt-4 space-y-4">
        {diary.is_locked && (
          <p className="text-xs text-purple-600 font-semibold">
            已上锁：除作者和维护者外任何人都无法查看此日记
          </p>
        )}
        {approvedEditors.length > 0 && (
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-gray-400 mb-2">共享作者</p>
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-4 sm:mb-6 lg:mb-8">
        <div>
          <p className="text-xs uppercase tracking-[0.5em] text-gray-400 mb-2">
            {isSelfDashboard ? 'My Dashboard' : 'Maintainer Console'}
          </p>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-gray-800">
            {ownerDisplayName ? `${ownerDisplayName} 的日记` : '日记管理'}
          </h1>
        </div>
      ...
      <Card className="mb-6 sm:mb-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-gray-400 mb-1">作者</p>
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-gray-400 mb-1">身份</p>
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.4em] text-gray-400 mb-1">日记数量</p>
          <p className="text-lg font-semibold text-gray-800">{diaries.length}</p>
          <p className="text-sm text-gray-500">当前列表中的日记</p>
        </div>
      </Card>
      ...
      {storageInfo && (
        <Card className="mb-6 sm:mb-8 p-5 sm:p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-gray-400 mb-2">存储配额</p>
              <div className="text-2xl font-bold text-gray-800">
                {formatBytes(storageInfo.used)} / {formatBytes(storageInfo.quota)}
              </div>
              <p className="text-xs text-gray-500 mt-1">单文件上限 {formatBytes(storageInfo.max)}</p>
            </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-between text-xs text-gray-500 mt-2">
              ...
              <span>剩余 {formatBytes(Math.max(0, storageInfo.quota - storageInfo.used))}</span>
            </div>
          </div>
        </Card>
      )}
      ...
      {isSelfDashboard && (
        <div className="mt-10">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold text-gray-800">我的协作日记</h2>
              ...
              {collaborations.map((diary) => (
                ...
                >
                  <p className="text-xs text-gray-500 mb-1">
                    原作者：{diary.owner?.display_name || diary.owner?.username || '未知'}
                  </p>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <p className="text-sm text-gray-600 line-clamp-2 flex-1 mr-4">
                      {diary.content.replace(/\[img:[^\]]+\]/g, '[图片]').substring(0, 80)}...
                    </p>
                    <Button
                      size="sm"
                      variant="ghost"
